...
// Outputs row-major matrix for row-vector convention (v * M).
static void build_projection_matrix(const XrFovf& fov, float near_z, float far_z, float out[4][4]) {
    ...
    // Column-major standard OpenGL-style projection, then transpose for row-major
    // We build it directly in row-major (row-vector) form:
    // In row-vector convention, clip = v * P, so P is transposed vs column-vector convention.
    memset(out, 0, sizeof(float) * 16);

    out[0][0] = 2.0f / width;
    out[1][1] = 2.0f / height;
    out[0][2] = (right + left) / width;
    out[1][2] = (up + down) / height;
    out[2][2] = -(far_z + near_z) / (far_z - near_z);
    out[2][3] = -1.0f;
    out[3][2] = -(2.0f * far_z * near_z) / (far_z - near_z);
}
...
static void pose_to_view_matrix(const XrPosef& pose, float world_scale, float out[4][4]) {
    ...
    // GLM is column-major, we need row-major for the engine's row-vector convention.
    // Transpose:
    for (int r = 0; r < 4; r++)
        for (int c = 0; c < 4; c++)
            out[r][c] = view[c][r];
...
bool vr_init() {
    ...
    // Prefer SRGB, fall back to UNORM
    int64_t chosen_format = DXGI_FORMAT_R8G8B8A8_UNORM;
    ...
    spdlog::info("[VR] Swapchain format: {}", chosen_format);
